...
export function ExpedienteScene({ profile, nextHref }: ExpedienteSceneProps) {
  const [portraitFailed, setPortraitFailed] = useState(false);
  const portraitAlt = `Retrato ilustrado de ${profile.fullName}`;
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0 space-y-2">
            <p className="font-mono text-xs font-semibold uppercase tracking-wider text-accent-primary">
              Registro de egreso · archivo de celebración
            </p>
            <h2
            ...
            <p className="max-w-prose text-sm leading-relaxed text-text-secondary">
              Una ficha breve para celebrar el recorrido que ya está listo para seguir avanzando.
            </p>
          </div>

          <div
            aria-label="Completitud del expediente: 100%"
            data-completion="100"
      ...
      <div className="grid min-w-0 gap-8 p-4 sm:p-6 md:grid-cols-5 md:p-8">
        <figure data-profile-field="portrait" className="min-w-0 space-y-3 md:col-span-2">
          ...
          <figcaption className="font-mono text-xs leading-relaxed text-text-muted">
            Retrato ilustrado · archivo de {profile.firstName}
          </figcaption>
        </figure>

        <div className="min-w-0 md:col-span-3">
          <p className="font-mono text-xs uppercase tracking-wider text-text-muted">
            Identificación celebrada
          </p>
          ...
          <dl className="mt-6 grid min-w-0 gap-3 sm:grid-cols-2">
            ...
            <div className="min-w-0 border-l-2 border-border-accent bg-surface-subtle px-3 py-3 sm:col-span-2">
              <dt className="font-mono text-xs uppercase tracking-wide text-text-muted">Estado</dt>
      ...
      <footer className="flex flex-col gap-4 border-t border-border-subtle p-4 sm:flex-row sm:items-center sm:justify-between sm:p-6 md:p-8">
        <p className="max-w-prose text-sm leading-relaxed text-text-secondary">
          Expediente completo. La próxima página registra cómo fue cambiando esta historia.
        </p>
        <a
          href={nextHref}
